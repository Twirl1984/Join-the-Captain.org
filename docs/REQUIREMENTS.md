...
- **REQ-WET-015** Maritime Windsymbole & Lesbarkeit (Status: umgesetzt) — Der Wind im Zeitreise-Overlay wird als Beaufort-Windfahne (Windbarb) dargestellt, die Richtung und Stärke in EINEM Glyph kodiert (Halbstrich 5 kn, Vollstrich 10 kn, Wimpel 50 kn; Schaft zeigt zur Wind-Herkunft, Windstille = Kreis). Symbole sind größer und kontrastreicher auf der OSM-Basiskarte; der Farbcode nach Böen-Stärke bleibt erhalten. Reine, injektionsfreie Ableitungen `windBarb`/`barbSvgPaths`/`windBarbRotationDeg`; hinter Flag `NEXT_PUBLIC_FEATURE_WX_SYMBOLS_V2` (Rückfall auf die bisherige Pfeil-Darstellung). Quelle: Nutzer-Feedback 2026-07-10 (Wetterzeichen/Wind auf der Karte, IMG_7035).
- **REQ-WET-016** Temperatur & Niederschlag als Kartensymbole (Status: umgesetzt) — Jeder Zeitschritt zeigt zusätzlich die 2-m-Temperatur (°C, ganzzahlig) und bei Niederschlag ein Regen-Glyph mit Intensitätsstufe (leicht/mäßig/stark aus mm/h); trocken zeigt kein Glyph, um die Karte nicht zu überladen. Das Datenmodell `TimelineStep` erhält `temperature_2m` und `precipitation` (Open-Meteo `temperature_2m`/`precipitation`); reine Formatierer `formatTemp`/`precipInfo`. Hinter Flag `NEXT_PUBLIC_FEATURE_WX_SYMBOLS_V2`. Quelle: Nutzer-Feedback 2026-07-10 (Wetterzeichen/Wind auf der Karte, IMG_7035).
...
- **REQ-NAV-021** GPX-Export (Status: umgesetzt) — Die geroutete Strecke lässt sich als GPX herunterladen (Plotter/Apps). Quelle: PDF-Plan-Abgleich 2026-07-10.
- **REQ-NAV-022** Törn-Stil und Tagesetappen (Status: geplant) — Skipper wählen einen Törn-Stil (z. B. „Urlaubstörn: Ankunft bis 17/18 Uhr, übernachten, weiter am Morgen" vs. „Durchfahren inkl. Nachtfahrt"); die Mehrtages-Planung teilt die Route entsprechend in Tagesetappen mit Stopp-Vorschlägen (verzahnt mit REQ-EXP-005/006). Zusätzlich: Wunsch-Ankunftszeit je Etappenziel („um 17 Uhr in der Bucht“) ergibt die empfohlene Abfahrtszeit je Tag (Rückwärtsrechnung über den Abfahrts-Scan); am Etappenziel wird die Bucht-/Hafen-Empfehlung (REQ-EXP-005) eingeblendet. Quelle: User 2026-07-10 (Video- und Test-Feedback).
- **REQ-NAV-023** Zeitabhängiges Wetterfeld fürs Profil-Routing (Status: umgesetzt) — Die Kantenkosten der Routen-Profile berücksichtigen die Wetterlage zur voraussichtlichen DURCHFAHRTSZEIT je Streckenabschnitt (heute: Feld zur Startzeit, dokumentierte Näherung; die ETA je Leg nutzt bereits die Leg-Zeit). Quelle: User 2026-07-10.
- **REQ-NAV-024** Informationsarchitektur: Törnplaner + Detail-Tools (Status: umgesetzt) — Die eine Seite wird in einen schlanken Törnplaner (Karte, Punkte, Berechnen, Profil) und abrufbare Sub-Tools gesplittet (Abfahrts-Scan, Zeitreise, Boot, Feedback als eigene Bereiche/Tabs; „Jetzt & hier“-Ansicht ab GPS-Position mit aktuellem Wetter). Progressive Disclosure nach docs/ux-konzept.md. Quelle: User 2026-07-10.
- **REQ-NAV-025** Handy-Peilung und Kreuzpeilung (Status: umgesetzt) — Objekt mit dem Handy anvisieren, Kompass-Heading einfrieren, Objekt auf der Karte zuordnen → Peillinie; zwei/drei Peilungen ergeben Standort bzw. Fehlerdreieck. Magnetisch→rechtweisend korrigiert (Missweisung je Revier), Fehlerband sichtbar, Safety-Wording. Design: docs/peilung.md. Quelle: User 2026-07-10.
- **REQ-NAV-026** GPS-Plausibilitätswarnung (Status: umgesetzt) — Die App erkennt unplausibles GPS (Abweichung vom Peilungs-Fix, Positionssprünge, accuracy-Ausreißer) und warnt aktiv statt still falsche Positionen zu zeigen. Quelle: User 2026-07-10.
- **REQ-NAV-027** AR-Kamera-Peilung (Status: geplant, Premium) — Optionale Zusatzvariante zur manuellen Peilung: Kamerabild öffnen, Kompassnadel/Peilstrahl einblenden, angeschautes Objekt im Kamerabild antippen → aus GPS-Position + Kompass-Heading wird ein Strahl gecastet und das nächste kartierte Objekt (Hafen/Seezeichen) auf der Karte gesucht und als Peilung-Referenz vorgeschlagen. Baut auf REQ-NAV-025 auf; die vereinfachte Peilung (Dropdown/Kartenklick) bleibt der Standardweg. Quelle: User 2026-07-13.
...
- **REQ-EXP-006** Törn-Vorschläge mit Highlights (Status: geplant) — Automatische Routen-Vorschläge je Revier/Zeitraum inkl. Erlebnissen und Festen; hoch bewertete POIs nahe der Ideallinie erzeugen eine Alternativ-Route mit Abstecher („Schönste vs. Direkte“, Wahl beim Skipper — POIs als Zwischen-Wegpunkte, nicht als A*-Kostenbonus). Quelle: User 2026-07-10, präzisiert durch PDF-Plan.
